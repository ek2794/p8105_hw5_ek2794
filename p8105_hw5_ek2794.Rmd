...
proptest(pull(baltimore, unsolved_sum), pull(baltimore, n_obs))
...
original dataset %>%
nest %>%
mutate %>% function within
##use nest, unnest for entire Df
output = map(list_norms, estimate, conf_low, conf_high)

output = vector("list")

for (i in 1:51) {
  output[[i]] = proptest(
    pull(crimes_unsolved, unsolved_sum),
    pull(crimes_unsolved, n_obs)
}

output = map(crimes_unsolved, proptest)
